...
def create_dagster_definitions() -> Definitions:
    ...
    resources = {
        "db": DatabaseResource(
            ...
            driver=EnvVar.str("DB_DRIVER", "ODBC Driver 17 for SQL Server"),
            pool_size=EnvVar.int("DB_POOL_SIZE", 10),
            pool_timeout=EnvVar.int("DB_POOL_TIMEOUT", 30)
        ),
        "fs": FileSystemResource(
            base_path=EnvVar("DATA_PATH"),
            encoding=EnvVar.str("FILE_ENCODING", "utf-8")
        ),
        "etl_config": ETLConfigResource(
            environment=EnvVar.str("ENVIRONMENT", "development"),
            debug_mode=EnvVar.bool("DEBUG_MODE", False),
            data_quality_threshold=EnvVar.float("DATA_QUALITY_THRESHOLD", 0.8)
        )
    }
...
def run_full_etl(ctx, data_path, connection_string, batch_size, skip_world_bank):
    ...
    try:
        ...
        if missing_files:
            click.echo(f"❌ Missing required files: {', '.join(missing_files)}", err=True)
            sys.exit(1)
    ...
    try:
        ...
        for filename in files_to_check:
            ...
            try:
                ...
                click.echo(f"  ⚠️  Null values: {validation_results[filename]['null_percentage']:.1f}%")
                click.echo(f"  ⚠️  Duplicates: {validation_results[filename]['duplicate_rows']:,}")
        ...
        total_files_ok = len([r for r in validation_results.values() if 'error' not in r])
...
def serve(port, host):
    ...
    try:
        ...
        # This would start the Dagster dev server
        click.echo("Note: Run 'dagster dev' directly for the full UI experience")
        
    except ImportError:
        click.echo("❌ Dagster development dependencies not available", err=True)
        click.echo("💡 Install with: pip install dagster[dev]")
        sys.exit(1)
