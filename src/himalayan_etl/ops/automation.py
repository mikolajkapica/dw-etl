...
class AutomationConfig(Config):
    """Configuration for automation operations."""
    ...
    try:
        ...
        for i, name in enumerate(unique_names):
            ...
            matches = process.extract(
                ...
                unique_names, 
                scorer=fuzz.ratio,
                limit=config.max_fuzzy_candidates
            )
            
            # Group names that meet the threshold
            similar_group = [name]
            for match_name, score in matches:
                if match_name != name and score >= config.fuzzy_match_threshold:
...
def load_incremental_data(
    ...
    try:
        ...
        if not inserts.empty:
            try:
                # Remove metadata columns before insert
