"""
Data cleaning operations for Himalayan Expeditions ETL pipeline.

This module contains Dagster ops for cleaning and standardizing data
from various sources, handling missing values, data type conversions,
and value standardization.
"""
...
def clean_expeditions_data(
    ...
    try:
        ...
        if duplicate_count > 0:
            ...
            df = df.sort_values(["EXPID", "_completeness_score"], ascending=[True, False])
        ...
        df["TOTAL_DEATHS"] = (df.get("MDEATHS", 0).fillna(0) + 
                             df.get("HDEATHS", 0).fillna(0))
        ...
        context.log.info(f"Rows: {initial_rows} → {final_rows} ({initial_rows - final_rows} removed)")
...
def clean_members_data(
    ...
    try:
        ...
        if "CITIZEN" in df.columns:
            ...
            df["CITIZEN_CODE"] = df["CITIZEN"].map(config.country_code_mapping).fillna(df["CITIZEN"])
        ...
        for col in status_fields:
            if col in df.columns:
                if col in ["DEATH"]:
                    # Convert to boolean
                    df[col] = df[col].astype(str).str.upper().isin(["TRUE", "1", "Y", "YES"])
        ...
        if invalid_count > 0:
            context.log.warning(f"Removing {invalid_count} members with insufficient data")
            df = df[valid_members]
...
def clean_peaks_data(
    ...
    try:
        ...
        if duplicate_count > 0:
            ...
            df["_completeness_score"] = df.notna().sum(axis=1)
            df = df.sort_values(["PEAKID", "_completeness_score"], ascending=[True, False])
        ...
        for col in height_columns:
            if col in df.columns:
                ...
                if col == "HEIGHTM":  # Height in meters
                    ...
                    if invalid_count > 0:
                        context.log.warning(f"Found {invalid_count} peaks with invalid heights")
        ...
        if "YRDSC" in df.columns:
            df["FIRST_ASCENT_YEAR"] = _convert_to_numeric(df["YRDSC"], "int", context, "YRDSC")
            # Validate reasonable year range
            current_year = datetime.now().year
            invalid_years = (df["FIRST_ASCENT_YEAR"] < 1800) | (df["FIRST_ASCENT_YEAR"] > current_year)
        ...
        if invalid_count > 0:
            context.log.warning(f"Removing {invalid_count} peaks with insufficient data")
            df = df[valid_peaks]
...
def _clean_date_column(series: pd.Series, context: OpExecutionContext, col_name: str) -> pd.Series:
    """Clean and convert date column to datetime format."""
    try:
        # Try multiple date formats
        date_formats = ["%Y-%m-%d", "%m/%d/%Y", "%d/%m/%Y", "%Y", "%m/%Y"]
        
        cleaned_series = pd.to_datetime(series, errors="coerce", infer_datetime_format=True)
        
        # If that fails, try explicit formats
        if cleaned_series.isna().all():
            for date_format in date_formats:
                try:
                    cleaned_series = pd.to_datetime(series, format=date_format, errors="coerce")
        ...
        if na_count > 0:
            context.log.warning(f"Could not parse {na_count} dates in column {col_name}")
...
def _convert_to_numeric(series: pd.Series, dtype: str, context: OpExecutionContext, col_name: str) -> pd.Series:
...
def _clean_name_field(series: pd.Series) -> pd.Series:
    ...
    cleaned = cleaned.str.replace(r"\bMc(\w)", r"Mc\1", regex=True)  # McDonald -> McDonald
    cleaned = cleaned.str.replace(r"\bO'(\w)", r"O'\1", regex=True)  # O'connor -> O'Connor
...
def _handle_missing_values(df: pd.DataFrame, context: OpExecutionContext) -> pd.DataFrame:
    """Handle missing values with appropriate strategies."""
...
def _validate_cleaned_data(df: pd.DataFrame, context: OpExecutionContext):
    ...
    for field in key_fields:
        if field in df.columns:
            ...
            if missing_count > 0:
                context.log.warning(f"Key field {field} has {missing_count} missing values")
