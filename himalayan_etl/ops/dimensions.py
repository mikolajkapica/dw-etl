...
from ..resources import DatabaseResource, ETLConfigResource
...
load_dimension_config_schema = {
    ...
    "key_columns": Field(DagsterArray(String), default_value=[], description="Columns used to identify existing records"),
    "update_columns": Field(DagsterArray(String), default_value=[], description="Columns to update (empty for all non-key columns)"),
}
        ...
        if not cleaned_expeditions.empty:
            ...
            for col in exp_cols:
                if col in cleaned_expeditions.columns:
                    exp_nations = cleaned_expeditions[col].dropna().astype(str).str.upper().str.strip()
        ...
        if not cleaned_members.empty:
            ...
            for col in member_cols:
                if col in cleaned_members.columns:
                    member_nations = cleaned_members[col].dropna().astype(str).str.upper().str.strip()
                    nationalities.update(member_nations[member_nations != "NAN"])
        
        # Remove empty strings and invalid entries
        nationalities = {n for n in nationalities if n and n not in ["", "NAN", "UNKNOWN", "NULL"]}
...
def create_dim_peak(
    ...
    try:
        ...
        if not cleaned_expeditions.empty and 'PEAKID' in cleaned_expeditions.columns:
            ...
            peak_countries = cleaned_expeditions.groupby('PEAKID')['HOST'].first().reset_index()
        ...
        if 'LAT' in dim_peak.columns and 'LON' in dim_peak.columns:
            dim_peak['Coordinates'] = dim_peak.apply(
                lambda row: f"{row['LAT']:.6f},{row['LON']:.6f}" 
        ...
        final_columns = [
            'PeakID', 'PeakName', 'HeightMeters', 'ClimbingStatus',
            'FirstAscentYear', 'HostCountryCode', 'Coordinates',
...
def create_dim_expedition_status(
    ...
    try:
        ...
        if cleaned_expeditions.empty or 'TERMREASON' not in cleaned_expeditions.columns:
            ...
            term_reasons = set(cleaned_expeditions['TERMREASON'].dropna().astype(str).str.upper().str.strip())
        ...
        for reason in sorted(all_reasons):
            ...
            mapping_info = config.termination_reason_mapping.get(reason, {
                "category": "Other",
                "is_success": False
            })
...
def load_dimension_table(
    ...
    """
    Load dimension data into SQL Server table using upsert logic.
    
    Performs insert for new records and update for existing records
    based on the specified key columns.
    
    Args:
        dimension_data: DataFrame with dimension data to load
        
    Returns:
        Dict with load results and statistics
    """
...
def _get_country_region(country_code: str) -> Tuple[str, str]:
    ...
    region_mapping = {
        # South Asia
        "NPL": ("Asia", "South Asia"), "IND": ("Asia", "South Asia"), 
        "PAK": ("Asia", "South Asia"), "BGD": ("Asia", "South Asia"),
        "BTN": ("Asia", "South Asia"), "LKA": ("Asia", "South Asia"),
        
        # East Asia
        "CHN": ("Asia", "East Asia"), "JPN": ("Asia", "East Asia"),
        "KOR": ("Asia", "East Asia"), "PRK": ("Asia", "East Asia"),
        "MNG": ("Asia", "East Asia"),
        
        # Southeast Asia
        "MMR": ("Asia", "Southeast Asia"), "THA": ("Asia", "Southeast Asia"),
        "VNM": ("Asia", "Southeast Asia"), "LAO": ("Asia", "Southeast Asia"),
        
        # Europe
        "GBR": ("Europe", "Western Europe"), "FRA": ("Europe", "Western Europe"),
        "DEU": ("Europe", "Western Europe"), "ITA": ("Europe", "Western Europe"),
        "RUS": ("Europe", "Eastern Europe"),
        
        # North America
        "USA": ("Americas", "North America"), "CAN": ("Americas", "North America"),
...
def create_dim_host_country(
    context: OpExecutionContext,
    enhanced_expeditions: pd.DataFrame
) -> pd.DataFrame:
    """
