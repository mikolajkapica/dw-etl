...
from himalayan_etl.ops.world_bank import (
    extract_world_bank_data as extract_world_bank_api,
...
def himalayan_etl_full_load():
    ...
    validation_results = validate_extracted_data(
        raw_expeditions, raw_members, raw_peaks, raw_references, raw_world_bank
    )
    ...
    enhanced_members = cleaned_members    # Step 3: Dimension Creation and Loading
    # Date dimension
    dim_date = create_dim_date(enhanced_expeditions)
    date_load_result = load_dim_date(dim_date)
    
    # Nationality dimension
    dim_nationality = create_dim_nationality(enhanced_expeditions, enhanced_members)
    nationality_load_result = load_dim_nationality(dim_nationality)
    
    # Peak dimension
    dim_peak = create_dim_peak(cleaned_peaks, enhanced_expeditions)
    peak_load_result = load_dim_peak(dim_peak)
    
    # Route dimension
    dim_route = create_dim_route(enhanced_expeditions)
    route_load_result = load_dim_route(dim_route)
      # Expedition status dimension
    dim_expedition_status = create_dim_expedition_status(enhanced_expeditions)
    status_load_result = load_dim_expedition_status(dim_expedition_status)
    
    # Host country dimension
    dim_host_country = create_dim_host_country(enhanced_expeditions)
    country_load_result = load_dim_host_country(dim_host_country)
    
    # Member dimension
    dim_member = create_dim_member(enhanced_members)
    member_load_result = load_dim_member(dim_member)
      # Step 4: World Bank Data (Optional)
    cleaned_wb_data = clean_world_bank_data(raw_world_bank)
...
def himalayan_etl_incremental():
    ...
    # Create all required dimensions for fact table processing
    ...
    # Load dimensions with upsert logic
    date_load_result = load_dim_date(dim_date)
    nationality_load_result = load_dim_nationality(dim_nationality)
    peak_load_result = load_dim_peak(dim_peak)
    route_load_result = load_dim_route(dim_route)
...
def daily_incremental_schedule(context: ScheduleEvaluationContext):
    ...
    return RunRequest(
        run_key=f"incremental_{context.scheduled_execution_time.strftime('%Y%m%d')}",
        run_config={
            "ops": {
